...
- [x] Dutch transcription re-check (all 81 Dutch records): **goetzmann0011** (Middelburg insurance Conditien) was genuine word-salad — a two-column print whose columns the OCR pipeline interleaved (museum-data.json field == scripts/ocr/0011.txt, same bad source). Regenerated from high-res column crops of the 4017×5969 original: full Dutch + English, 20 articles, ~10.3k chars. **goetzmann0507** (Amsterdam Prys-Courant, 16 May 1822) also regenerated from high-res crops — full domestic + foreign securities sections (Amerika/Rusland/Denemarken/Oostenryk/Italie/Sicilie/Spanje/Groot-Brittannie/Frankryk/Bentinck/Baden/Columbia) + Wissel-/Specie-Cours; old "16 October" was a hallucination (sheet reads 16 Mey). 
- [x] Search corpus updated: `scripts/ocr/goetzmann0011.txt` + `0507.txt` rewritten (plain text so section/country names survive `clean_for_index`), `scripts/corpus.db` rebuilt via `build_corpus_index.py` (545 docs, 186k words, 866 box files). Verified FTS hits (Bentinck/Oostenryk→0507, assurantiekamer→0011) and garbled terms gone.
- [x] Workbook consolidation: `oov_data_master.xlsx` was byte-identical to `oov_data_new.xlsx`, so it was removed as redundant. **`oov_data_new.xlsx` is now the single canonical workbook.** Both rebuild scripts (`_rebuild_master2.py`, `_rebuild_master.py`) default to writing it in place.
- [x] Legacy-essay metadata backfilled into JSON (0900/0904/0908/0909/0910): the 5 essays had null issueYear/currency/language/issuingCountry/subjectCountry/creator while the Excel retained them. Backfilled 30 fields from the Excel with proper Unicode restored (Städtisches, Société, Compañía, Västindiska; 0909 creator filled from issuer, 0910 cleaned). Re-synced both xlsx so JSON↔Excel agree.
